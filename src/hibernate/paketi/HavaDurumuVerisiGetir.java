...
public class HavaDurumuVerisiGetir {
    public  static List<HavaDurumu> databaseVeriGetir(String sehir) {
        ...
        try {
            ...
            havaDurumuList=  session.createQuery("from HavaDurumu  where sehir=?").setParameter(0,sehir).getResultList();
            session.getTransaction().commit();
            System.out.println("HavaDurumu bilgileri liste olarak databaseden alındı!");

        }
        catch (Exception e) {
            System.out.println("İstediğiniz şehre ait hava durumu bulunamadı...");
            havaDurumuList=null;
        }
        finally {
            session.close();
